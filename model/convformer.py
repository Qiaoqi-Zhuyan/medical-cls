...
class ConvFormerBlock(nn.Module):
    ...
    def __init__(self, dim, mode="conv", ffn_ratio=4,
        ...
        assert mode == "conv" or mode == "attn", "mode should be conv or attn"

        self.norm1 = norm_layer(dim)
        self.token_mixer = self._conv_mixer if mode == "conv" else self._attn_mixer
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * ffn_ratio)
        '''
        in_channel, out_channel, expan_ratio=4
        '''
        self.ffn = ConvFFN(in_channel=dim, out_channel=dim, expan_ratio=ffn_ratio)
